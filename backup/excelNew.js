    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (isDropDown) {
                ...
                // // Add a hidden worksheet for dropdown options
                // const tagSheet = workbook.addWorksheet('tagDropDown', { state: 'hidden' });
                // tagSheet.columns = [{ header: 'tag', width: 30 }];

                // // Populate the hidden worksheet with dropdown options
                // dropdownData?.tag?.forEach((option, index) => {
                //     tagSheet.getCell(`K${index + 1}`).value = option || "";
                // });
                ...
                // const tagDropDownRange = `tagDropDown!$K$1:$K$${dropdownData?.tag?.length}`;
                ...
                if (data?.length > 0) {
                    data.forEach((val, index) => {
                        ...
                        const tagCell = row.getCell('L'); // Assuming 'Tag' is column K
                        if (val?.tagColor) {
                            tagCell.fill = {
                                type: 'pattern',
                                pattern: 'solid',
                        ...
                        const statusCell = row.getCell('N'); // Assuming 'Status' is column M
